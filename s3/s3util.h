...
#include <tuple>
#include <vector>
#include "s3/aws_s3_rate_limiter.h"
...
class S3Util {
  ...
  class CutomizedS3Client: public S3Client {
   public:
    CutomizedS3Client(
            ...
            const string& secret_key): S3Client(
                Aws::Auth::AWSCredentials(access_id, secret_key),
 ...
 private:
  explicit S3Util(const string& endpoint,
                  const string& access_id,
                  const string& secret_key,
                  const string& bucket,
    ...
    s3Client = std::make_unique<CutomizedS3Client>(client_config, access_id, secret_key);
    Aws::StringStream ss;
