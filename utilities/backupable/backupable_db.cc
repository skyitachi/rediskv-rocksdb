    ...
    io_s = status_to_io_status(checkpoint.CreateCustomCheckpoint(
        [&](const std::string& src_dirname, const std::string& fname,
          ...
          io_st = db_fs_->GetFileSize(src_dirname + "/" + fname, io_options_,
                                      &size_bytes, nullptr);
          if (!io_st.ok()) return io_st;

          std::string dst_relative = GetPrivateFileRel(new_backup_id, false, fname);
          ...
          io_st = backup_fs_->LinkFile(src_dirname + "/" + fname, final_dest_path,
                                       io_options_, nullptr);
          if (io_st.ok()) {
  ...
  for (auto& item : backup_items_to_finish) {
    ...
    if (item_io_status.ok()) {
      if (item.shared && item.needed_to_copy) {
        ...
        item_io_status = item.backup_env->GetFileSystem()->RenameFile(
            item.dst_path, item.dst_path, io_options_, nullptr);
      }
    }
    if (item_io_status.ok()) {
